...
이번 작업에서 가장 크게 확인한 점은 Rule Engine을 바꾸지 않아도 지식 품질을 개선할 수 있다는 것이었다. 검색 알고리즘이나 엔진 구조는 그대로 두고, 기존 Rule JSON을 updated note 기준으로 재정렬한 뒤 테스트로 검증하는 것만으로도 Rule 기반 구조의 신뢰도를 높일 수 있었다.
